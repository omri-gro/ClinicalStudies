...
from stats_sandbox import get_equivocal_zone_masks, get_interval_bin_masks, binary_classification_metrics_bootstrap
...
# version of generate_fda_equivocal_report that uses central bins as well
# currently separate as needs different format of decision_dict
def generate_fda_equivocal_report_bins(methd_comp, decision_dict, level_a='REF', level_b='TEST', dim_col='Method'):
    """
    Study-specific orchestrator for the BMA FDA MDL request.
    Handles single thresholds and multi-step interval middle bins natively.
    """
    rows = []
    for variable, bins in decision_dict.items():
        try:
            ...
            print(f"Skipping {variable} in FDA Report: {e}")
            continue

        for lower, upper, lower_inc, upper_inc, label in bins:

            # Pass boundaries directly to the updated math tool
            y_true_bin, y_pred_bin, exclude_mask = get_interval_bin_masks(
                x, y,
                lower_cutoff=lower,
                upper_cutoff=upper,
                lower_inc=lower_inc,
            ...
            # 3. Adjusted Metrics (Equivocal Grey Zone Dropped)
            y_true_adj = y_true_bin[~exclude_mask]
            y_pred_adj = y_pred_bin[~exclude_mask]
            df_adj = df_strat[~exclude_mask].copy()
            ...
            # 4. Compile detailed results row
            row = {
                "Variable": variable,
                "Clinical Bin": label,
                "Range": f"[{lower}, {upper}]" if lower_inc else f"({lower}, {upper}]",
                "N (Strict)": len(x),
                "N (Adjusted)": len(y_true_adj),
                "Equivocal Cases Excluded": exclude_mask.sum(),

                "Strict TP": strict_metrics["tp"], "Strict FP": strict_metrics["fp"],
                "Strict FN": strict_metrics["fn"], "Strict TN": strict_metrics["tn"],
                "Strict Sensitivity (%)": f"{strict_metrics['sensitivity']['value'] * 100:.1f} ({strict_metrics['sensitivity']['ci'][0] * 100:.1f}-{strict_metrics['sensitivity']['ci'][1] * 100:.1f})" if pd.notna(
                    strict_metrics['sensitivity']['value']) else "NA",
                "Strict Specificity (%)": f"{strict_metrics['specificity']['value'] * 100:.1f} ({strict_metrics['specificity']['ci'][0] * 100:.1f}-{strict_metrics['specificity']['ci'][1] * 100:.1f})" if pd.notna(
                    strict_metrics['specificity']['value']) else "NA",
            ...
            if adj_metrics:
                row.update({
                    "Adjusted TP": adj_metrics["tp"], "Adjusted FP": adj_metrics["fp"],
                    "Adjusted FN": adj_metrics["fn"], "Adjusted TN": adj_metrics["tn"],
                    "Adjusted Sensitivity (%)": f"{adj_metrics['sensitivity']['value'] * 100:.1f} ({adj_metrics['sensitivity']['ci'][0] * 100:.1f}-{adj_metrics['sensitivity']['ci'][1] * 100:.1f})" if pd.notna(
                        adj_metrics['sensitivity']['value']) else "NA",
                    "Adjusted Specificity (%)": f"{adj_metrics['specificity']['value'] * 100:.1f} ({adj_metrics['specificity']['ci'][0] * 100:.1f}-{adj_metrics['specificity']['ci'][1] * 100:.1f})" if pd.notna(
                        adj_metrics['specificity']['value']) else "NA",
